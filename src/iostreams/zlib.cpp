...
zlib_error::zlib_error(int error) : BOOST_IOSTREAMS_FAILURE("zlib error"), error_(error) {}
...
zlib_base::zlib_base() : stream_(new z_stream), calculate_crc_(false), crc_(0), crc_imp_(0), total_in_(0), total_out_(0) {}
...
void zlib_base::before(const char *&src_begin, const char *src_end, char *&dest_begin, char *dest_end)
{
...
void zlib_base::after(const char *&src_begin, char *&dest_begin, bool compress)
{
	...
	if (calculate_crc_) {
		const zlib::byte *buf = compress ? reinterpret_cast<const zlib::byte *>(src_begin) : reinterpret_cast<const zlib::byte *>(const_cast<const char *>(dest_begin));
		zlib::uint length = compress ? static_cast<zlib::uint>(next_in - src_begin) : static_cast<zlib::uint>(next_out - dest_begin);
...
void zlib_base::reset(bool compress, bool realloc)
{
	...
	realloc ? (compress ? deflateReset(s) : inflateReset(s)) : (compress ? deflateEnd(s) : inflateEnd(s));
	//);
	crc_imp_ = 0;
}

void zlib_base::do_init(const zlib_params &p, bool compress, zlib::xalloc_func /* alloc */, zlib::xfree_func /* free*/, void *derived)
{
	...
	zlib_error::check BOOST_PREVENT_MACRO_SUBSTITUTION(compress ? deflateInit2(s, p.level, p.method, window_bits, p.mem_level, p.strategy) : inflateInit2(s, window_bits));
}
