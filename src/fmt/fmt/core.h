...
#define FMT_USE_CONSTEXPR11 (FMT_MSC_VER >= 1900 || FMT_GCC_VERSION >= 406 || FMT_USE_CONSTEXPR)
#endif
...
template<typename Char> FMT_CONSTEXPR size_t length(const Char *s)
{
	const Char *start = s;
	while (*s)
		++s;
...
public:
    ...
    \endrst
   */
	FMT_CONSTEXPR basic_string_view(const Char *s) : data_(s), size_(internal::length(s)) {}

	/** Constructs a string reference from a ``std::basic_string`` object. */
	template<typename Alloc> FMT_CONSTEXPR basic_string_view(const std::basic_string<Char, Alloc> &s) FMT_NOEXCEPT : data_(s.c_str()), size_(s.size()) {}
...
public:
	explicit container_buffer(Container &c) : basic_buffer<typename Container::value_type>(&c[0], c.size(), c.size()), container_(c) {}
};
...
//   fmt::format(L"{}", L"test");
template<typename Char> inline void require_wchar()
{
...
template<typename T, typename Char> struct is_named_arg<named_arg<T, Char>> : std::true_type {};
...
template<typename T, typename Char, bool ENABLE = true> struct convert_to_int {
...
template<typename Context, type TYPE> struct typed_value : value<Context> {
	static const type type_tag = TYPE;

	template<typename T> FMT_CONSTEXPR typed_value(const T &val) : value<Context>(val) {}
};

template<typename Context, typename T> FMT_CONSTEXPR basic_format_arg<Context> make_arg(const T &value);

#define FMT_MAKE_VALUE(TAG, ArgType, ValueType)                                        \
	template<typename C> FMT_CONSTEXPR typed_value<C, TAG> make_value(ArgType val) \
	{                                                                              \
		return static_cast<ValueType>(val);                                    \
	}

#define FMT_MAKE_VALUE_SAME(TAG, Type)                                              \
	template<typename C> FMT_CONSTEXPR typed_value<C, TAG> make_value(Type val) \
	{                                                                           \
		return val;                                                         \
	}
...
template<typename C> inline typed_value<C, char_type> make_value(wchar_t val)
{
...
// iostreams.
template<typename C, typename T> typename std::enable_if<!std::is_same<T, typename C::char_type>::value>::type make_value(const T *)
{
	static_assert(!sizeof(T), "formatting of non-void pointers is disallowed");
}

template<typename C, typename T>
inline typename std::enable_if<std::is_enum<T>::value && convert_to_int<T, typename C::char_type>::value, typed_value<C, int_type>>::type make_value(const T &val)
{
	return static_cast<int>(val);
}

template<typename C, typename T, typename Char = typename C::char_type>
inline typename std::enable_if<std::is_constructible<basic_string_view<Char>, T>::value, typed_value<C, string_type>>::type make_value(const T &val)
{
...
template<typename C, typename T> typed_value<C, named_arg_type> make_value(const named_arg<T, typename C::char_type> &val)
{
...
private:
	...
	template<typename Visitor, typename Ctx> friend FMT_CONSTEXPR typename internal::result_of<Visitor(int)>::type visit(Visitor &&vis, basic_format_arg<Ctx> arg);
...
protected:
	...
	context_base(OutputIt out, basic_string_view<char_type> format_str, basic_format_args<Context> ctx_args) : parse_context_(format_str), out_(out), args_(ctx_args) {}
...
template<typename Context, typename Arg, typename... Args> FMT_CONSTEXPR11 unsigned long long get_types()
{
...
template<bool IS_PACKED, typename Context, typename T> inline typename std::enable_if<IS_PACKED, value<Context>>::type make_arg(const T &value)
{
	return make_value<Context>(value);
}

template<bool IS_PACKED, typename Context, typename T> inline typename std::enable_if<!IS_PACKED, basic_format_arg<Context>>::type make_arg(const T &value)
{
...
private:
	...
	static FMT_CONSTEXPR11 long long get_types() { return IS_PACKED ? static_cast<long long>(internal::get_types<Context, Args...>()) : -static_cast<long long>(NUM_ARGS); }
...
/**
  ...
  \endrst
 */
template<typename Context, typename... Args> inline format_arg_store<Context, Args...> make_format_args(const Args &...args)
{
	return format_arg_store<Context, Args...>(args...);
}

template<typename... Args> inline format_arg_store<format_context, Args...> make_format_args(const Args &...args)
{
...
public:
   ...
   \endrst
   */
	template<typename... Args> basic_format_args(const format_arg_store<Context, Args...> &store) : types_(static_cast<unsigned long long>(store.TYPES))
	{
...
struct format_args : basic_format_args<format_context> {
	template<typename... Args> format_args(Args &&...arg) : basic_format_args<format_context>(std::forward<Args>(arg)...) {}
};
struct wformat_args : basic_format_args<wformat_context> {
	template<typename... Args> wformat_args(Args &&...arg) : basic_format_args<wformat_context>(std::forward<Args>(arg)...) {}
};
...
template<typename... Args> inline void print_colored(color c, string_view format_str, const Args &...args)
{
	vprint_colored(c, format_str, make_format_args(args...));
}
template<typename... Args> inline void print_colored(color c, wstring_view format_str, const Args &...args)
{
...
// character type.
template<typename S, typename Enable = void> struct format_string_traits {
private:
...
template<typename Char> struct format_string_traits_base {
	typedef Char char_type;
};

template<typename Char> struct format_string_traits<Char *> : format_string_traits_base<Char> {};

template<typename Char> struct format_string_traits<const Char *> : format_string_traits_base<Char> {};

template<typename Char, std::size_t N> struct format_string_traits<Char[N]> : format_string_traits_base<Char> {};

template<typename Char, std::size_t N> struct format_string_traits<const Char[N]> : format_string_traits_base<Char> {};

template<typename Char> struct format_string_traits<std::basic_string<Char>> : format_string_traits_base<Char> {};

template<typename Char> struct format_string_traits<basic_string_view<Char>> : format_string_traits_base<Char> {};

template<typename S> struct is_format_string : std::integral_constant<bool, std::is_constructible<format_string_traits<S>>::value> {};

template<typename S> struct is_compile_string : std::integral_constant<bool, std::is_base_of<compile_string, S>::value> {};
...
typename std::enable_if<is_contiguous<Container>::value, std::back_insert_iterator<Container>>::type vformat_to(std::back_insert_iterator<Container> out, string_view format_str,
														format_args args)
{
...
template<typename Container>
typename std::enable_if<is_contiguous<Container>::value, std::back_insert_iterator<Container>>::type vformat_to(std::back_insert_iterator<Container> out, wstring_view format_str,
														wformat_args args)
{
	internal::container_buffer<Container> buf(internal::get_container(out));
	vformat_to(buf, format_str, args);
...
inline typename std::enable_if<is_contiguous<Container>::value, std::back_insert_iterator<Container>>::type format_to(std::back_insert_iterator<Container> out,
...
inline std::basic_string<typename internal::format_string_traits<String>::char_type> format(const String &format_str, const Args &...args)
{
...
/**
  ...
  **Example**::
  ...
  \endrst
 */
template<typename... Args> inline void print(std::FILE *f, string_view format_str, const Args &...args)
{
 ...
 */
template<typename... Args> inline void print(std::FILE *f, wstring_view format_str, const Args &...args)
{
...
template<typename... Args> inline void print(wstring_view format_str, const Args &...args)
{
	format_arg_store<wformat_context, Args...> as(args...);
	vprint(format_str, as);
}
